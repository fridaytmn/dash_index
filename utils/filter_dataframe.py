...
def filter_dataframe(
    filter_value: str, included_columns, dataframe: pd.DataFrame
) -> pd.DataFrame:
...
def filter_dataframe_by_column(
    filter_query: str, dataframe: pd.DataFrame
) -> pd.DataFrame:
...
def filter_column(filter_part: str, dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    if operator == "contains":
        return dataframe.loc[
            dataframe[col_name]
            .astype(str)
            .str.contains(re.escape(str(filter_value)), flags=re.IGNORECASE, regex=True)
        ]
...
def filter_query_parser(
    ...
    real_operator = OPERATORS.get(
        filter_part.replace("{" + column_name + "}", "").split()[:1][0], None
    )
    if value[0] == value[-1] and value[0] in ("'", '"', "`"):
        return (
            column_name,
            real_operator,
            value[1:-1].replace("\\" + value[0], value[0]),
        )
    try:
...
def use_comparison_operators(
    operator: str, dataframe: pd.DataFrame, col_name: str, filter_value: str
) -> pd.DataFrame:
