...
def update(_, sticker, count, cell):
    ...
    if count and (cell != get_value_by_location(FILE_PATH, row=location[0], column=location[1] + 2)):
        values = [str(count), cell]
    else:
        values = [str(count), get_value_by_location(FILE_PATH, row=location[0], column=location[1] + 2)]
...
@app.callback(
    ...
    Output(component_id="save_new_storage_sticker", component_property="children", allow_duplicate=True),
