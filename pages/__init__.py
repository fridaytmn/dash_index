...
DATE_FORMAT_FOR_CELERY = (
    "%d-%m-%Y %H:%M"  # Дата для отображения в поле статуса отчета полученного из селери
)
DATE_FORMAT_BAR = "%b-%Y"
...
ENTITY_TYPE_LABEL_LEGAL = (
    "юр"  # тип субъекта правовых отношений "юридическое лицо", вариант на русском
)
ENTITY_TYPE_LABEL_PHYSICAL = (
    "физ"  # тип субъекта правовых отношений "физическое лицо", вариант на русском
)
ENTITY_TYPE_LABEL_OTHER = (
    "другой"  # неопределенный тип субъекта правовых отношений, вариант на русском
)
ENTITY_TYPE_LABEL_ALL = (
    "все"  # все типы субъекта правовых отношений, вариант на русском
)
ENTITY_TYPE_LEGAL = (
    "legal"  # тип субъекта правовых отношений "юридическое лицо", вариант на английском
)
...
ENTITY_TYPE_OTHER = (
    "other"  # неопределенный тип субъекта правовых отношений, вариант на английском
)
...
USER_TYPE_PAYING_OR_NON_PAYING = (
    "is_payer in [True, False]"  # любой пользователь, вне зависимости от покупки
)
...
SELECT_OPTIONS_SHOW_UNVISITED_URLS = (
    1  # значение при выборе параметра Показывать непосещенные URL - да
)
SELECT_OPTIONS_DONT_SHOW_UNVISITED_URLS = (
    0  # значение при выборе параметра Показывать непосещенные URL - нет
)
REGIONS_ALL = "all"  # выбор всех регионов, английский вариант
EMPTY_ROW = ""  # пустая строка
QUERIES_LABEL_ZERO_AND_PARTIAL_SUGGESTION = (
    "нулевые + неточный"  # нулевые поисковые запросы и неточный поиск на сайте
)
...
schema = Schema(
    ...
    tags=KEYWORD(
        lowercase=True, analyzer=StemmingAnalyzer(stemfn=stem), field_boost=0.5
    ),
)
storage = RamStorage()
...
for page in pages_provider:
    writer.add_document(
        ...
        tags=[tag.value for tag in page.get_tags()],
    )
